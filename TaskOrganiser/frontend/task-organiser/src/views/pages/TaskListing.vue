...
const loadNodes = (first, rows) => {
    ...
    let completable = filterStatus.value.useFilter ?
        taskService.queryTasksWithChildStatus(taskModel.value.parentCode, getFilterQueryBody()) :
        taskService.getAllTasks(taskModel.value.parentCode);
    completable.then((tasks) => {
...
function isLeaf(task) {
    return countTotalChildren(task) === 0;
}

function countTotalChildren(task) {
    let total = 0;

    task.childrenStats?.forEach((child) => {
        total += child.count;
    });
...
<template>
    <div>
        <div class="card">
            ...
            <TreeTable :value="nodes" :lazy="true" :paginator="false" :rows="rows" @nodeExpand="onExpand"
                       :row-hover="true" tableStyle="min-width: 50rem">
